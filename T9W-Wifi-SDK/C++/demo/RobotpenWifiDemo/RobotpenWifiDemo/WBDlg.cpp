...
void CWBDlg::readIni()
{
	m_nState = 270;//GetPrivateProfileInt(_T("General"),_T("State"),0,GetAppPath() + _T("\\USBHelper.ini"));
	m_nPenWidth = 1.2;//GetPrivateProfileInt(_T("General"),_T("PenWidth"),1,GetAppPath() + _T("\\USBHelper.ini"));

	m_nWidth = 22600;//GetPrivateProfileInt(_T("General"),_T("Width"),0,GetAppPath() + _T("\\USBHelper.ini"));
	m_nHeight = 16650;//GetPrivateProfileInt(_T("General"),_T("Height"),0,GetAppPath() + _T("\\USBHelper.ini"));
}
...
BOOL CWBDlg::OnEraseBkgnd(CDC* pDC)
{
	// TODO: 在此添加消息处理程序代码和/或调用默认值
	CBrush bk(RGB(255, 255, 255));  
	CBrush* pold=pDC->SelectObject(&bk);  
	CRect rect;  
	pDC->GetClipBox(&rect);  
